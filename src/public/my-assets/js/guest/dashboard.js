    ...
    const DashboardController = {
        ...
        EventListener() {
            $('#input_magazine').change(function () {
                ...
                DashboardController.GetStatisticalContributionFaculties(magazine_id)
                    .then((response) => {
                        setTimeout(() => {
                            const {faculty_name, statistical_contribute_faculty_data} = response
                            DashboardController.RenderContributeFacultyChart(faculty_name, statistical_contribute_faculty_data)
                            Loading(false)
                        }, 1500)
                    })
            })
        },
        RenderContributeFacultyChart(faculty_name, statistical_contribute_faculty_data) {
            const data_chart = {
                labels: [faculty_name],
                datasets: [{
                    label: "Contribution",
                    data: [statistical_contribute_faculty_data],
        ...
        GetStatisticalContributionFaculties(magazine_id) {
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: `/guest/dashboard/statistical-contribution-faculty/${magazine_id}`,
                    type: 'GET',
        ...
        GetStatisticalContributeYears() {
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: `/guest/dashboard/statistical-contribution-years`,
                    type: 'GET',
                    success: function (response) {
                        resolve(response)
                    },
        ...
        GetFirstStatistical() {
            ...
            Promise.all([contribute_of_faculty, contribute_of_years])
                .then((results) => {
                    setTimeout(() => {
                        const {faculty_name, statistical_contribute_faculty_data} = results[0]
                        DashboardController.RenderContributeFacultyChart(
                            faculty_name, statistical_contribute_faculty_data
                        )
